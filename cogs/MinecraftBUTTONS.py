        ...
        while True:            
            ...
            try:
                ...
                if move == "⏸️":
                    ...
                    await res.respond(content='paused', type=7, components=[])
                    pause_menu_buttons = [
                        [
                            Button(style=ButtonStyle.grey, label='🔁'),
                            Button(style=ButtonStyle.grey, label='🚫'),
                    ...
                    printed_pause_menu = await ctx.send(embed=embed, components=pause_menu_buttons)
                    ...
                    res2 = await self.bot.wait_for("button_click", check=check, timeout=30)
                
                    print(res2.component.label)
                ...
                if move == "🗺️":
                    if mapview == False:
                        ...
                        mapview = True
                        #print(big_line)
                        await res.respond(content=big_line, type=7)
                        #await printed_field.edit(content=big_line)
                    else:
                        await res.respond(content=boardfield(field, x, y), type=7)
                        #await printed_field.edit(content=boardfield(field, x, y))
                        #await printed_field.remove_reaction("🗺️", self.bot.get_user(ctx.author.id))
                        mapview = False
